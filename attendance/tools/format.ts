    export default function format(name: string) {
        try {
            if(name) {
                ...
                var middle_ini = first_name.split(" ").find(item => item.toLowerCase() === temp_name[temp_name.length-1].toLowerCase()) ? "." : " " + temp_name[temp_name.length-1].toUpperCase() + "."; //Checks if last index is equal to first name or middle initial
